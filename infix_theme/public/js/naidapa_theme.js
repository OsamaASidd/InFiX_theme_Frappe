(function () {
    ...
    frappe.provide("infix_theme");

    infix_theme.setup = function () {
        $('body').addClass('infix-theme-active');
        infix_theme.run_patches();
    };

    infix_theme.setup_icon_picker = function () {
    ...
    infix_theme.show_icon_dialog = function () {
        const icons = [
    ...
    infix_theme.run_patches = function () {
        infix_theme.highlight_active_route();
        infix_theme.mutate_workspace_container();
        infix_theme.mutate_custom_elements();
        infix_theme.inject_navbar_toggle();
        infix_theme.mutate_number_cards();
        infix_theme.setup_icon_picker();
    };

    infix_theme.mutate_number_cards = function () {
        $('.number-widget-box').each(function (index) {
            $(this).attr('data-color-index', index % 4);
        });
    };

    infix_theme.inject_navbar_toggle = function () {
    ...
    infix_theme.mutate_custom_elements = function () {
        const changes = [
    ...
    infix_theme.mutate_workspace_container = function () {
        const selectors = [
    ...
    infix_theme.mutate_charts = function () {
        ...
        $('.frappe-chart').each(function () {
            try {
                ...
                if (chart && !chart._infix_splined) {
                    chart._infix_splined = true;
    ...
    $(document).ready(() => {
        // Ensure audio elements exist for frappe.utils.play_sound
        var sounds = [
        ...
        sounds.forEach(function (s) {
            if (!document.getElementById("sound-" + s.name)) {
                var el = document.createElement("audio");
                el.id = "sound-" + s.name;
                el.setAttribute("volume", s.volume);
                el.preload = "auto";
                el.innerHTML = '<source src="' + s.src + '">';
